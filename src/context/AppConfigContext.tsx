...
export interface AppConfig {
  feature_focus_enabled: boolean; feature_review_enabled: boolean
  feature_mocks_enabled: boolean; feature_mcq_enabled: boolean
  feature_companions_enabled: boolean; feature_insights_enabled: boolean
  feature_buddy_enabled: boolean
  maintenance_mode: boolean; maintenance_message: string
  signup_active: boolean; max_users: number; trial_days: number
  default_daily_goal_hours: number; xp_multiplier: number; base_price: number
  global_free_mode: boolean; free_mode_message: string
}
const DEFAULT: AppConfig = {
  feature_focus_enabled:true, feature_review_enabled:true, feature_mocks_enabled:true,
  feature_mcq_enabled:true, feature_companions_enabled:true, feature_insights_enabled:true,
  feature_buddy_enabled:true, maintenance_mode:false, maintenance_message:'',
  signup_active:true, max_users:500, trial_days:14, default_daily_goal_hours:6,
  xp_multiplier:1, base_price:299, global_free_mode:false,
  free_mode_message:'🎉 Early access — everything is free for now!',
}
type Ctx = { config: AppConfig; loading: boolean; refresh: () => Promise<void> }
const AppConfigContext = createContext<Ctx>({ config: DEFAULT, loading: true, refresh: async () => {} })
