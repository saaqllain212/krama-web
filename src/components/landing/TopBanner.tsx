...
export default function TopBanner() {
  ...
  useEffect(() => { if (!localStorage.getItem('kb-v4')) setVisible(true) }, [])
  ...
  return (
    <div className="relative z-50 bg-gradient-to-r from-emerald-600 via-green-500 to-emerald-600 text-white py-2.5 px-4 text-center">
      <span className="inline-flex items-center gap-2 flex-wrap justify-center text-xs font-bold uppercase tracking-widest">
        <span className="animate-pulse w-2 h-2 bg-white rounded-full opacity-80 inline-block" />
        🎉 Early Access — Every feature FREE · No credit card · 500 student cap
        <span className="animate-pulse w-2 h-2 bg-white rounded-full opacity-80 inline-block" />
      </span>
      <button onClick={() => { setVisible(false); localStorage.setItem('kb-v4','1') }}
